...
root_dir = fullfile('.','tests','results','generic_architecture');
load(fullfile(root_dir,'results_test_generic_architecture.mat'));
...
flg_KLD = 0;
flg_wrapup = 0;
fusion_methods = {'Integrator'};
nFusionMethods = 1;
source_names = {'Source1','Source2','Source3','Source4'};
N_sources = length(source_names);

% get parameters
DST_param_file         = './parameters/Generic_Architecture.prm';
DST_param_file_content = read_parameters(DST_param_file);
FOD_set_file           = './parameters/FOD_set.prm';
FOD_set_file_content   = read_parameters(FOD_set_file);
fused_FOD              = DST_param_file_content.parameters.integrator.FOD_set;
fused_FOD_set          = unique(struct2cell(FOD_set_file_content.parameters.(fused_FOD)));
N_classes_fused        = length(fused_FOD_set);

% N truth tracks
truth_FOD       = 'feature1_FOD';
truth_FOD_set   = unique(struct2cell(FOD_set_file_content.parameters.(truth_FOD)));
N_trks          = 2*length(truth_FOD_set);
N_obj_trks      = N_trks;
N_classes_truth = length(truth_FOD_set);
truth_class     = [1:N_classes_truth, 1:N_classes_truth];

% class mapper
class_mapper = FOD_mapping_class(FOD_set_file);
...
if flg_wrapup
    ...
    for iFM = 1 : nFusionMethods
        % Populate with algorithm information
        this_conf = results.conf_mat_fuse_agg(:,:);
        this_CPV = reshape(results.CPV_fuse(:,:,:),N_tracks,N_classes,N_MC);
        this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
        wrap_up_table(iFM,2:end) = easyWrapUp(this_conf,this_CPV,class_mapper,truth_FOD,fused_FOD);
    end
    for is = 1 : N_sources
        ...
        this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
        wrap_up_table(nFusionMethods+is,2:end) = easyWrapUp(this_conf,this_CPV,class_mapper,truth_FOD,fused_FOD);
    end
    ...
    for is = 1 : N_sources
        ...
        for iFM = 1 : nFusionMethods
            % given source
            this_conf = results.conf_mat_fuse_src_agg(:,:,is);
            this_CPV = reshape(results.CPV_fuse_src(:,:,is,:),N_tracks,N_classes,N_MC);
            this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
            wrap_up_table_given_src(iFM,2:end) = easyWrapUp(this_conf,this_CPV,class_mapper,truth_FOD,fused_FOD);
            
            % given not source
            this_conf = results.conf_mat_fuse_agg(:,:) - results.conf_mat_fuse_src_agg(:,:,is,iFM);
            this_CPV = reshape(results.CPV_fuse(:,:,:),N_tracks,N_classes,N_MC);
            ...
            wrap_up_table_given_NOT_src(iFM,2:end) = easyWrapUp(this_conf,this_CPV,class_mapper,truth_FOD,fused_FOD);
        end
...
if flg_CM
    ...
    for iFM = 1 : nFusionMethods
        ...
        this_conf = results.conf_mat_fuse_agg(:,:,iFM);
        title_str = [fusion_methods{iFM},' Confusion Matrix'];
        easyPlotCM(this_conf,title_str);
        fig_name = fullfile(saveDir,[fusion_methods{iFM},' Confusion Matrix.png']);
        saveas(fig,fig_name);
        close(fig);
        ...
        for is = 1 : N_sources
            ...
            fig = figure('color','white');
            this_conf = results.conf_mat_fuse_src_agg(:,:,is,iFM);
            title_str = {[fusion_methods{iFM},' Confusion Matrix'],['Given ', source_names{is},' Reported']};
            easyPlotCM(this_conf,title_str);
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' Confusion Matrix Given ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
            
            %% algorithm confusion matrix given source didn't report
            fig = figure('color','white');
            this_conf = results.conf_mat_fuse_agg(:,:,iFM) - results.conf_mat_fuse_src_agg(:,:,is,iFM);
            title_str = {[fusion_methods{iFM},' Confusion Matrix'],['Given ', source_names{is},' Didn''t Report']};
            easyPlotCM(this_conf,title_str);
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' Confusion Matrix Given Not ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
...
if flg_ROC
    ...
    for iFM = 1 : nFusionMethods
        ...
        this_CPV = reshape(results.CPV_fuse(:,:,:),N_tracks,N_classes,N_MC);
        this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
        title_str = [fusion_methods{iFM},' Modified ROC Curve'];
        easyPlotMroc(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
        fig_name = fullfile(saveDir,[fusion_methods{iFM},' Modified ROC Curve.png']);
        saveas(fig,fig_name);
        close(fig);
        ...
        for is = 1 : N_sources
            ...
            this_CPV = reshape(results.CPV_fuse_src(:,:,is,:),N_tracks,N_classes,N_MC);
            this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
            title_str = {[fusion_methods{iFM},' Modified ROC Curve'],['Given ', source_names{is},' Reported']};
            easyPlotMroc(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' Modified ROC Curve Given ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
            
            %% algorithm ROC given source didn't report
            fig = figure('color','white');
            this_CPV = results.CPV_fuse;
            source_CPV = reshape(results.CPV_src(:,:,is,:),N_tracks,N_classes,N_MC);
            this_CPV(:,:,~isnan(reshape(source_CPV(1,1,:),1,N_MC))) = [];
            this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,size(this_CPV,3)))) = [];
            title_str = {[fusion_methods{iFM},' Modified ROC Curve'],['Given ', source_names{is},' Didn''t Report']};
            easyPlotMroc(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' Modified ROC Curve Given Not ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
...
if flg_pDist
    ...
    for iFM = 1 : nFusionMethods
        ...
        this_CPV = reshape(results.CPV_fuse(:,:,:),N_tracks,N_classes,N_MC);
        this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
        title_str = [fusion_methods{iFM},' Probability Distribution'];
        easyPlotpdist(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
        fig_name = fullfile(saveDir,[fusion_methods{iFM},' pDist.png']);
        saveas(fig,fig_name);
        close(fig);
        ...
        for is = 1 : N_sources
            ...
            this_CPV = reshape(results.CPV_fuse_src(:,:,is,:),N_tracks,N_classes,N_MC);
            this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
            title_str = {[fusion_methods{iFM},' Probability Distribution'],['Given ', source_names{is},' Reported']};
            easyPlotpdist(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
            saveDir = fullfile(root_dir,'Pdistribution',source_names{is});
            if ~isfolder(saveDir)
                mkdir(saveDir);
            end
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' pDist Given ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
            ...
            this_CPV(:,:,~isnan(reshape(source_CPV(1,1,:),1,N_MC))) = [];
            this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,size(this_CPV,3)))) = [];
            title_str = {[fusion_methods{iFM},' Probability Distribution'],['Given ', source_names{is},' Didn''t Report']};
            easyPlotpdist(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' pDist Given Not ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
...
if flg_KLD
    ...
    for iFM = 1 : nFusionMethods
        ...
        this_CPV = reshape(results.CPV_fuse(:,:,:),N_tracks,N_classes,N_MC);
        this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
        title_str = [fusion_methods{iFM},' Prediction vs Observation'];
        easyPlotKLD(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
        fig_name = fullfile(saveDir,[fusion_methods{iFM},' KLD.png']);
        saveas(fig,fig_name);
        close(fig);
        ...
        for is = 1 : N_sources
            ...
            this_CPV = reshape(results.CPV_fuse_src(:,:,is,:),N_tracks,N_classes,N_MC);
            this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,N_MC))) = [];
            title_str = {[fusion_methods{iFM},' Prediction vs Observation'],['Given ', source_names{is},' Reported']};
            easyPlotKLD(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' KLD Given ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
            ...
            this_CPV(:,:,~isnan(reshape(source_CPV(1,1,:),1,N_MC))) = [];
            this_CPV(:,:,isnan(reshape(this_CPV(1,1,:),1,size(this_CPV,3)))) = [];
            title_str = {[fusion_methods{iFM},' Prediction vs Observation'],['Given ', source_names{is},' Didn''t Report']};
            easyPlotKLD(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD);
            fig_name = fullfile(saveDir,[fusion_methods{iFM},' KLD Given Not ',source_names{is},'.png']);
            saveas(fig,fig_name);
            close(fig);
...
function wrap_up_table_entry = easyWrapUp(this_conf,this_CPV,class_mapper,truth_FOD,fused_FOD)
% initialize
[N_truth_classes,N_call_classes] = size(this_conf);
N_call_classes = N_call_classes - 2;
P_correct = NaN(1,1);
P_error = NaN(1,1);
P_NoCall = NaN(1,1);
P_NoReport= NaN(1,1);
P_CAL = NaN(1,N_call_classes);
P_KLD = NaN(1,1);
ROC_area = NaN(1,N_call_classes);
wrap_up_table_entry = table(P_correct,P_error,P_NoCall,P_NoReport,P_KLD,P_CAL,ROC_area);

% get basic information
mappingMAT = class_mapper.buildMappingMatrix(truth_FOD,fused_FOD);
mapped_conf = mappingMAT*this_conf;
N_correct = sum(diag(mapped_conf(1:N_call_classes,1:N_call_classes)));
N_total_calls = sum(sum(mapped_conf(1:N_call_classes,1:N_call_classes)));
N_NoCalls = sum(mapped_conf(:,N_call_classes+1));
N_NoReports = sum(mapped_conf(:,N_call_classes+2));
...
% P_KLD
res = easyDetermineKLDmetrics(this_CPV,class_mapper,truth_FOD,fused_FOD);
wrap_up_table_entry(1,5) = {exp(-res.DS)};
% P_CAL
wrap_up_table_entry(1,6) = {exp(-abs(res.REL)')};
% ROC area
[~,~,ROC_area] = easymROCcurve(this_CPV,1,class_mapper,truth_FOD,fused_FOD);
...
function easyPlotMroc(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD)
[~,N_classes,~] = size(this_CPV);
[fnr,fpr,~,~,~,truth,prediction] = easymROCcurve(this_CPV,0,class_mapper,truth_FOD,fused_FOD);
legend_str = [];
hold off;
if~isempty(truth)
...
function [fnr,fpr,area,tpr,thresholds,truth,prediction] = easymROCcurve(this_CPV,flg_area,class_mapper,truth_FOD,fused_FOD)
[N_tracks,N_classes,N_MC] = size(this_CPV);
prediction = zeros(N_classes,N_MC*N_tracks);
truth = zeros(N_classes,N_MC*N_tracks);

mappingMAT = class_mapper.buildMappingMatrix(truth_FOD,fused_FOD);
N_truth_classes = size(mappingMAT,2);
...
for it = 1 : N_tracks
    ...
    iClass = mod(it-1,N_truth_classes) + 1;
    iClass_mapped = find(mappingMAT(:,iClass));
    truth(iClass_mapped,i:endi) = true;
...
function easyPlotpdist(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD)
if isempty(this_CPV)
    return;
end
...
for it = 1 : N_tracks
    this_class = mod(it-1,N_truth_classes) + 1;
    this_class_mapped = find(mappingMAT(:,this_class));
    prior_class_count(this_class_mapped) = prior_class_count(this_class_mapped)+1;
end
for ic = 1 : N_classes
    subplot(dim,dim,ic);
    hold off
    ...
    for it = 1 : N_tracks
        this_class = mod(it-1,N_truth_classes)+1;
        this_class_mapped = find(mappingMAT(:,this_class));
        iend = i(this_class_mapped) + N_MCs;
        i(this_class_mapped) = i(this_class_mapped) + 1;
        class_P(this_class_mapped,i(this_class_mapped):iend) = this_CPV(it,ic,:);
        i(this_class_mapped) = iend;
    end
    legend_str = [];
    hold off
    for ic2 = 1 : N_classes
...
function easyPlotKLD(this_CPV,title_str,class_mapper,truth_FOD,fused_FOD)
[~,N_classes,~] = size(this_CPV);
colors = nncolor.ncolors(N_classes);
res = easyDetermineKLDmetrics(this_CPV,class_mapper,truth_FOD,fused_FOD);
hold off
...
for ic = 1 : N_classes
    ...
    scatter(res.prob_bins(idx_not_zero),res.ok(ic,idx_not_zero,ic)...
        ,max(5,(res.ok_weight(ic,idx_not_zero,ic)*200).^2),colors(ic,:),'filled'...
        ,'MarkerFaceAlpha',.6);
    hold on
end
...
function res = easyDetermineKLDmetrics(this_CPV,class_mapper,truth_FOD,fused_FOD)
...
for ic = 1 : N_classes
    ...
    for k1 = 1 : length(prob_bins)
        if ~isnan(ok(1,k1,ic))
            ...
            if ok(ic,k1,ic)
                N_times = N_times + 1;
                REL(ic) = REL(ic) + ok_weight(1,k1,ic) * KLD(pk1,ok(ic,k1,ic));
                weight_tot = weight_tot + ok_weight(1,k1,ic);
            end
        end
    end
